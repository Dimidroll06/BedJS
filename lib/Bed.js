var BedJS = function(params = {}){
    ...
    this.layer = function(params = {}){
        /*
            ...
            visible = true
            paralaxX = 1 - value from 0 to 1
            paralaxy = 1 - value from 0 to 1
        */
        ...
        this.paralaxX = typeof params.paralaxX !== 'undefined'? params.paralaxX : 1
        this.paralaxY = typeof params.paralaxY !== 'undefined'? params.paralaxY : 1
    ...
    this.rect = function(lay, params={}){
        ...
        this.destroy = ()=>{
            lay.sprites.splice(lay.sprites.indexOf(this), 1)
            delete this
        }
        ...
        this.draw= ()=>{
            if(this.visible){
                ...
                ctx.fillRect(this.x - (scrollX * layr.paralaxX), 
                this.y - (scrollY * layr.paralaxY), 
        ...
        this.draw = ()=>{
            ...
            if(this.visible) {
                if(currentAnimation == 'default') {
                    ...
                    this.x - (scrollX * layr.paralaxX), this.y - (scrollY * layr.paralaxY), 
                    this.image.width * (this.size/100), 
                    this.image.height * (this.size/100))
                } 
                else{
    ...
    this.text = function(lay,params={}){
        ...
        */
        ...
        this.draw = function(){
            ...
            ctx.fillText(this.words, this.x - (scrollX * layr.paralaxX), this.y - (scrollY * layr.paralaxY))
        }
    ...
    //TileMap
    this.tilemap = function(lay, params={}){

        this.x = (params.x!=undefined)?params.x:0
        this.y = (params.y!=undefined)?params.y:0
        this.width = (params.width!=undefined)?params.width:32
        this.height = (params.height!=undefined)?params.height:32

        this.visible = (typeof params.visible === 'undefined'?true:params.visible)

        this.image = new Image()
        this.image.src = './lib/logo_sheet.png' || params.src || params.image

        this.verticalElements = 2 || params.verticalElements
        this.horizontalElements = 2 || params.horizontalElements
            ...
            if(this.visible){
                for (let x = 0; x < this.tiles.length; x++) {
                    ...
                    for (let y = 0; y < this.tiles[x].length; y++) {
                        const elem = this.tiles[y][x]
                        
                        var TileW = this.image.width / this.horizontalElements
                        var TileH = this.image.height / this.verticalElements
                        
                        ctx.drawImage( this.image, 
                            elem.x * TileW, elem.y * TileH, TileW, TileH,
                            this.x + (x * this.width) + (scrollX * layer.paralaxX), this.y + (y * this.height) + (scrollY * layer.paralaxY),
                            this.width, this.height
                        )
